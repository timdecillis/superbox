...
export default EmailModal = ({email, setEmail, emailModalOpen, setEmailModalOpen}) => {
      ...
      >
        <GlobalViewFlat style={styles.modalContainer}>
          <GlobalViewFlat style={styles.modalContent}>
            ...
            <TextInput onChangeText={(text) => setEmail(text)} autoFocus={true} style={styles.findUser}></TextInput>
            <GlobalViewFlat style={styles.buttons}>
              ...
              <GlobalCartButton onPress={() => {
                updatePersonal(profile.firebase_uid, profile.idToken, 'email', email);
                closeModal();
