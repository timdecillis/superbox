...
export const updatePersonal = (uid, token, field, update) => {
    console.dir('IDIDIDID:::' + uid + 'AUTHAUTH:' + token)
    let data = {
        [field]: update
    };
    console.log(data);
    return axios.put(baseUrl + `/api/u/users/${uid}/profile`, data, {headers: {'authentication': token}})
};
export const banUser = (uid, token) => {
    console('ID:', uid)
    return axios.put(baseUel + `/api/u/users/${uid}/profile`, {headers: {'authentication': token}})
};
